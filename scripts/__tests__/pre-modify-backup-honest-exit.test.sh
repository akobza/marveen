...
# The fixture is written through python3, like the snapshot itself: the sqlite3 CLI
# is not an install dependency, and the script no longer calls it (card 252ab361).
...
# The script takes its database snapshot through python3 and lists the snapshot
# with find, sed and tr (card 252ab361), so those belong on the curated PATH; the
# sqlite3 CLI does not, because nothing calls it any more.
for c in bash python3 find mv tr cp mkdir grep cut date du ls tail rm dirname basename git wc sed awk head sort uname; do
...
for need in python3 find grep cp; do
