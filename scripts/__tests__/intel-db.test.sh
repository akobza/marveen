...
# Read the database the way the product does, not through a CLI that is not
# installed. The sqlite3 binary is NOT an install dependency (card 252ab361), so
# every `sqlite3 ...` below used to exit 127 and this file was permanently red --
# which meant every later verdict had to step over a known failure first.
sqldb() { node "$INSTALL_DIR/scripts/lib/sqlite-cli.mjs" "$@"; }
...
COUNT=$(sqldb "$INTEL_DB" "SELECT COUNT(*) FROM known_facts_registry")
STATUS=$(sqldb "$INTEL_DB" "SELECT status FROM known_facts_registry WHERE id='$ID1'")
...
for t in watchlist active_focus decision_log; do
  N=$(sqldb "$INTEL_DB" "SELECT COUNT(*) FROM $t")
  if [ "$N" = "1" ]; then
    pass "$t has 1 row"
  else
...
sqldb "$INTEL_DB" "UPDATE known_facts_registry SET status='closed' WHERE id='$ID1'"
sqldb "$INTEL_DB" "UPDATE active_focus SET expires_at=1"
