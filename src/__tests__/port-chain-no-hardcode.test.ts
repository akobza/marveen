...
describe('PORTCHAIN1: the port chain follows WEB_PORT on a NON-default port', () => {
  ...
  it('leaves the documented fallbacks alone', () => {
    const stays: Array<[string, RegExp]> = [
      // b2cd0f43 changed the SHAPE, not the fallback, and then changed it once more.
      // The 3420 default is still here and still the last resort; it now goes through
      // resolveWebPort(), which validates via parseWebPort() and -- deliberately --
      // FALLS BACK instead of throwing, exporting the rejected value as WEB_PORT_INVALID
      // for the two gates that refuse on it. So this pattern pins two things more than
      // the original did: that the default is last, and that it is reached through the
      // validating resolver rather than assigned raw.
      ['src/config.ts', /resolveWebPort\([^)]*3420\)/],
      ['src/config.ts', /return \{ port: parseWebPort\(/],
